...
from database import (
    ...
    get_all_users, get_db_connection, get_group_by_id, create_group
)
...
def render_chat_list():
    ...
    if groups:
        ...
        for group in groups:
            ...
            with col1:
                if st.button(f"Group: {group['name']}{unread_badge}", 
...
def render_active_chat():
    ...
    # Кнопка управления группой (если это группа)
    if chat['type'] == 'group':
...
def render_message(msg, is_my_message):
    """Рендерит одно сообщение с красивой версткой"""
    # Форматируем время
    ...
    # Имя отправителя
    sender_name = "You" if is_my_message else msg['username']
...
def send_message_form(chat_type, chat_id):
    """Форма для отправки сообщений и файлов"""
    user_id = st.session_state['user_id']
    
    # Проверяем права на отправку
    if st.session_state['role'] == 'guest' and chat_type != 'general':
        st.warning("Guests can only write in general chat!")
        return
    
    # Определяем, куда отправляем сообщение
    to_group_id = None
    to_user_id = None
...
def show_welcome_screen():
    """Приветственный экран при выборе чата"""
    st.markdown("""
    <div style="
        display: flex;
...
def show_group_management():
    """Управление группой - добавление и удаление участников"""
    ...
    # Может управлять: создатель или админ
    can_manage = is_creator or is_admin_user
    
    # Панель управления группой (в сайдбаре)
    with st.sidebar:
        ...
        if not members:
            ...
            for member in members:
                ...
                with col3:
                    ...
                    # Правила удаления:
                    # 1. Админ может удалить любого
                    # 2. Создатель может удалить любого (кроме себя)
                    # 3. Обычный участник может только выйти сам
                    ...
                    if is_admin_user:
                        ...
                        can_remove = True  # Выйти из группы
                    ...
                    if can_remove:
                        ...
                        if st.button(button_text, key=f"remove_{member['id']}", help=button_help):
                            # Нельзя удалить создателя (если ты не админ)
                            if member['id'] == created_by and not is_admin_user:
                                ...
                                if member['id'] == current_user_id:
                                    # Если вышел сам - закрываем управление и чат
                                    del st.session_state['manage_group']
        ...
        # Добавление новых участников (только для создателя или админа)
        if can_manage:
            ...
            if available_users:
                ...
                if st.button("Add to Group", use_container_width=True):
                    ...
                    st.success(f"User added to group!")
                    st.rerun()
            else:
...
# Функция для проверки, является ли пользователь создателем группы
...
# Функция для получения количества участников группы
